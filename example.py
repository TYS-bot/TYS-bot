...
from customreward import KickoffProximityReward, ZeroSumReward, SwiftGroundDribbleReward, AirTouchReward, CradleFlickReward, LemTouchBallReward, RetreatReward, DistanceReward, AerialDistanceReward, InAirReward, TouchVelChange, GoodVelocityPlayerToBallReward, CradleReward, GroundedReward, GroundDribbleReward, JumpTouchReward
from rlgym_sim.utils.reward_functions import CombinedReward
from rlgym_sim.utils.reward_functions.common_rewards import (
VelocityBallToGoalReward, EventReward, FaceBallReward, SaveBoostReward, TouchBallReward, LiuDistanceBallToGoalReward, 
    AlignBallGoal
)
...
def build_rocketsim_env():
    ...
    reward_fn = LogCombinedReward.from_zipped(
        ...
        (EventReward(team_goal=2.35, concede=-1.2, boost_pickup=0.1), 150),
        #(TouchVelChange(threshold=500), 50),
        (JumpTouchReward(min_height=120), 20),
        
                            #CONTINOUS REWARDS:
        (VelocityBallToGoalReward(), 15),
        (TouchBallReward(aerial_weight=4), 6),
        (KickoffProximityReward(), 25),
        (GoodVelocityPlayerToBallReward(), 3),
        (FaceBallReward(), .001),
        (SaveBoostReward(), 4),
        (AerialDistanceReward(5, 10), 1.5),
        (LemTouchBallReward(), 3.5),
        (InAirReward(), .097),
        (ZeroSumReward(SwiftGroundDribbleReward(), 0, 2.0), 1),
        (ZeroSumReward(CradleFlickReward(), 0, 2.0), 2.5)
    )
